...
## Implementation
This algorithm relies on the pretrained VGG19 model. We do not use the fully-connected part of the model, we only use certain layers of the convolutional part the model i.e.
...
That means that if we choose our content layer to be the layer which is higher in the hierarchy it would lead to image objects keeping their basic shape, but with some distortions. </br>
